...
def validate_file(file):
    ...
    if file_type not in ALLOWED_TYPES:
        st.error(f"File '{file.name}' must be PNG or JPG.")
        return False
    try:
        file.seek(0)
            ...
            for file_name in zip_ref.namelist():
                if file_name.lower().endswith(('.png', '.jpg', '.jpeg')):
                    ...
                    if validate_file(file_buffer):
                        uploaded_count += load_images_to_db([file_buffer], folder, base_name)
    except Exception as e:
...
if st.session_state.zoom_folder is None:
    ...
    for f in data:
        ...
        if images:
            ...
            # Image Grid
            cols = st.columns(4)
            for idx, img_dict in enumerate(sorted(images, key=lambda x: x["sequence"], reverse=True)):  # Descending order
                with cols[idx % 4]:
